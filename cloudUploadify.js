...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="up_filename">${fileName}</div><div class="uploadify-progress"><span class="uploadify-progress-bar"></span></div><div class="uploadBtnDiv"><span class="uploadbtn">上传</span></div><div class="delFileBtnDiv"><span class="delfilebtn">删除</span></div></div>';
	var defaults = {
	...
	this.each(function(){
		...
		$('#tablecontainer').append(uploadFileListStr);	//存放表格的位置
	  ...
	  fileObj = {
		  ...
		  fileInput:$("#geoEastFileInput"),//改写后file控件
		  uploadFileList : $('#tablecontainer').find('.uploadify-queue'),//_this.find('.uploadify-queue'),//自己修改后
		  ...
		  funSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					var file = files[i];
					//处理模板中使用的变量
					var $html = $(option.itemTemplate.replace(/\${fileID}/g,'fileupload_'+instanceNumber+'_'+file.index).replace(/\${fileName}/g,file.name).replace(/\${fileSize}/g,formatFileSize(file.size)));
					//如果是自动上传，去掉上传按钮
					if(option.auto){
					...
					var len = 0
					$.each($("#tablecontainer .uploadify-queue-item"), function(index, obj) {
					...
					$("#uploadNum").text(len);
					$("#uploadBadge").text(len);

					
					
					//判断是否显示已上传文件大小
					if(option.showUploadedSize){
						var num = '<div class="progressnum"><i class="uploadedsize">'+initFileSize+'</i>/<i class="totalsize">${fileSize}</i></div>'.replace(/\${fileSize}/g,formatFileSize(file.size));
					...
					if(option.showUploadedPercent){
						...
						var percentText = '<div class="up_percentDiv"><i class="up_percent">'+initUppercent+'</i><div>';
		  ...
		  onProgress: function(file, loaded, total) {
				var eleProgress = $('#tablecontainer').find('#fileupload_'+instanceNumber+'_'+file.index+' .uploadify-progress');
		  ...
		  funGetProgressWidth: function(index){
		  	var eleProgressBar = $('#tablecontainer').find('#fileupload_'+instanceNumber+'_'+index+' .uploadify-progress-bar');
		  ...
		  funDelUploadedSize:function(file){
		  	if(option.saveInfoLocal){
		  ...
		  funDealFiles: function(files) {
			  var fileCount = $('#tablecontainer').find('.uploadify-queue .uploadify-queue-item').length;//队列中已经有的文件个数
			  for(var i=0,len=files.length;i<len;i++){
		  ...
		  funDeleteFile: function(index) {
			  for (var i = 0,len=this.fileFilter.length; i<len; i++) {
					  ...
					  if (file.index == index) {
						  ...
						  $('#tablecontainer').find('#fileupload_'+instanceNumber+'_'+index).fadeOut();
						  $('#uploadNum').text($('#uploadNum').text() - 1);
						  $('#uploadBadge').text($('#uploadBadge').text() - 1);
		  ...
		  funUploadFile: function(file) {
			  ...
			  var thisfile = $('#tablecontainer').find('#fileupload_'+instanceNumber+'_'+file.index);
					  ...
					  if (xhr.readyState == 4) {
						  ...
						  if (xhr.status == 200) {
								...
								if(fileObj.uploadOver){
								  ...
								  //在指定的间隔时间后删掉进度条
								  fileObj.funDelUploadedSize(originalFile.name);
								  setTimeout(function(){
								  	$('#tablecontainer').find('#fileupload_'+instanceNumber+'_'+originalFile.index).fadeOut();
		  ...
		  init: function() {	  
				...
				if (this.fileInput.length>0) {
				  	this.fileInput.accept = getAcceptString(option.fileTypeExts);
				...
				_this.on('click',function(){
					$('#geoEastFileInput').trigger('click');
				})
